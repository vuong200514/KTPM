...
DrawRectangle(x1, y1, x2, y2, color, alpha := 1, &parentWindow := "") {
    if (parentWindow && !parentWindow.HasProp("hwnd")) {
        throw Error(
            "The 'parentWindow' argument needs to have proper 'hwnd' property (for example a Gui object has it). Skip this value to create one."
        )
    }
...
GetPixelColorBuffered(x, y, refreshRate := 150, waitForUpdate := false) {
    ...
    UpdateScreenBuffer(windowHandle := 0) {
        ...
        DllCall("BitBlt", "Uint", myDC, "int", 0, "int", 0, "int", screenW, "int", screenH, "Uint", screenDC, "int",
            screenX, "int", screenY, "Uint", 0x40000000 | 0x00CC0020)
